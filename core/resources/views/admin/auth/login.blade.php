...
@section('content')
<div class="admin-login-page" style="background-image: url('{{ asset('assets/admin/images/login.jpg') }}')">
    ...
    <div class="admin-login-container">
        <div class="admin-login-card">
            <div class="admin-login-card__header">
                ...
                <h1 class="admin-login-card__title">@lang('Welcome to') <strong>{{ __($general->site_name) }}</strong></h1>
                <p class="admin-login-card__subtitle">{{ __($pageTitle) }} @lang('to') {{ __($general->site_name) }} @lang('Dashboard')</p>
            </div>
            <div class="admin-login-card__body">
                <form action="{{ route('admin.login') }}" method="POST" class="admin-login-form verify-gcaptcha">
                    @csrf
                    <div class="admin-login-form__group">
                        <label class="admin-login-form__label" for="admin-username">@lang('Username')</label>
                        <div class="admin-login-form__input-wrap">
                            <i class="las la-user admin-login-form__icon"></i>
                            <input type="text" id="admin-username" class="admin-login-form__input" value="{{ old('username') }}" name="username" placeholder="@lang('Enter your username')" required autocomplete="username">
                        </div>
                    </div>
                    <div class="admin-login-form__group">
                        <label class="admin-login-form__label" for="admin-password">@lang('Password')</label>
                        <div class="admin-login-form__input-wrap">
                            <i class="las la-lock admin-login-form__icon"></i>
                            <input type="password" id="admin-password" class="admin-login-form__input" name="password" placeholder="@lang('Enter your password')" required autocomplete="current-password">
                        </div>
                    </div>
                    <x-captcha />
                    <div class="admin-login-form__options">
                        <label class="admin-login-form__remember">
                            ...
                            <span class="admin-login-form__remember-text">@lang('Remember Me')</span>
                        </label>
                        <a href="{{ route('admin.password.reset') }}" class="admin-login-form__forgot">@lang('Forgot Password?')</a>
                    </div>
        ...
        <p class="admin-login-footer">@lang('Secure admin access') &middot; {{ __($general->site_name) }}</p>
    </div>
</div>

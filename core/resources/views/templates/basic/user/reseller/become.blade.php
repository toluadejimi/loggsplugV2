    ...
    <div class="row g-4 p-3 p-md-4">
        ...
        <div class="col-lg-9">
            <div class="reseller-hero reseller-hero--centered mb-4">
                ...
                <h1 class="reseller-hero__title">@lang('Reseller')</h1>
            ...
            {{-- Generate your own API key (self-service) --}}
            <div class="reseller-card reseller-card--highlight mb-4">
                <div class="reseller-card__body">
                    <h3 class="reseller-card__heading mb-2"><i class="las la-key me-2"></i>@lang('Generate your own API key')</h3>
                    ...
                    <form method="post" action="{{ route('user.reseller.generate-key') }}" class="d-inline">
                        @csrf
                        <button type="submit" class="btn btn-primary btn-lg"><i class="las la-plus me-2"></i>@lang('Generate API key')</button>
                    </form>
                </div>
            </div>
            ...
            <div class="reseller-card reseller-card--form mb-4">
                <div class="reseller-card__body">
                    <h3 class="reseller-card__heading mb-3"><i class="las la-server me-2"></i>@lang('Do you need Web installation? Pay 20k for installation on your server')</h3>
                    <p class="reseller-card__desc mb-4">Submit your details and we’ll contact you for payment (20,000) and setup on your server.</p>
                    <form method="post" action="{{ route('user.reseller.become.submit') }}">
                        @csrf
                        <div class="row g-3">
                            <div class="col-md-6">
                                <label class="form-label">@lang('Business name')</label>
                                <input type="text" name="business_name" class="form-control" value="{{ old('business_name') }}" required placeholder="Your business or brand name">
                                @error('business_name')<span class="text-danger small">{{ $message }}</span>@enderror
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">@lang('Contact name')</label>
                                <input type="text" name="contact_name" class="form-control" value="{{ old('contact_name', $user->fullname) }}" required>
                                @error('contact_name')<span class="text-danger small">{{ $message }}</span>@enderror
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">@lang('Contact email')</label>
                                <input type="email" name="contact_email" class="form-control" value="{{ old('contact_email', $user->email) }}" required>
                                @error('contact_email')<span class="text-danger small">{{ $message }}</span>@enderror
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">@lang('Contact phone')</label>
                                <input type="text" name="contact_phone" class="form-control" value="{{ old('contact_phone') }}" placeholder="Optional">
                                @error('contact_phone')<span class="text-danger small">{{ $message }}</span>@enderror
                            </div>
                            <div class="col-12">
                                <label class="form-label">@lang('Message')</label>
                                <textarea name="message" class="form-control" rows="3" placeholder="Tell us about your business or how you plan to resell (optional)">{{ old('message') }}</textarea>
                                @error('message')<span class="text-danger small">{{ $message }}</span>@enderror
                            </div>
                            <div class="col-12">
                                <button type="submit" class="btn btn-dark btn-lg"><i class="las la-paper-plane me-2"></i>@lang('Request Web installation')</button>
                            </div>
                        </div>
                    </form>
                </div>

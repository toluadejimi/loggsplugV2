            ...
            <div class="reseller-sidebar">
                <div class="reseller-sidebar__inner">
                    ...
                    <div class="reseller-user-card">
                        ...
                        <h6 class="reseller-user-card__name">{{ $user->fullname }}</h6>
                        <p class="reseller-user-card__meta">{{ $user->email }}</p>
                        <a href="{{ route('user.profile.setting') }}" class="reseller-user-card__link"><i class="las la-pencil-alt"></i> @lang('Edit profile')</a>
                    </div>
                    <nav class="reseller-nav">
                        <span class="reseller-nav__title">@lang('Menu')</span>
                        <a href="{{ route('user.home') }}" class="reseller-nav__item"><i class="las la-th-large"></i> @lang('Dashboard')</a>
                        <a href="{{ route('user.reseller.index') }}" class="reseller-nav__item active"><i class="las la-store"></i> @lang('Reseller')</a>
                        <a href="{{ route('user.reseller.api-docs') }}" class="reseller-nav__item"><i class="las la-book"></i> @lang('API Docs')</a>
                        <a href="{{ route('products') }}" class="reseller-nav__item"><i class="las la-shopping-bag"></i> @lang('Products')</a>
                        <a href="{{ route('user.logout') }}" class="reseller-nav__item"><i class="las la-sign-out-alt"></i> @lang('Logout')</a>
                    </nav>
                </div>
            </div>
            <div class="d-lg-none mt-2">
                <button type="button" class="btn btn-outline-dark w-100 show-reseller-sidebar"><i class="las la-bars me-2"></i>@lang('Menu')</button>
            </div>
        </div>
        <div class="col-lg-9">
            <div class="reseller-hero mb-4">
                <h1 class="reseller-hero__title">@lang('Reseller Dashboard')</h1>
                <p class="reseller-hero__sub">Manage your API key, mini-site, and payouts in one place.</p>
            </div>

            @if(session('notify'))
                @foreach(session('notify') as $n)
                    <div class="alert alert-{{ $n[0] === 'error' ? 'danger' : 'success' }} alert-dismissible fade show" role="alert">{{ __($n[1]) }}<button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>
                @endforeach
            @endif
                ...
                <div class="col-12">
                    <div class="reseller-card">
                        ...
                        <div class="reseller-card__body">
                            <h3 class="reseller-card__heading">@lang('Connect with API Key')</h3>
                            <p class="reseller-card__desc">Use this key in your app or mini website. Keep it private.</p>
                            <div class="reseller-apikey">
                                <code id="apiKeyDisplay" class="reseller-apikey__value">{{ $reseller->api_key }}</code>
                                <button type="button" class="btn btn-primary reseller-apikey__btn" id="copyApiKey"><i class="las la-copy me-1"></i>@lang('Copy')</button>
                            </div>
                            <form method="post" action="{{ route('user.reseller.generate-key') }}" class="mt-2 js-regenerate-key-form">
                                @csrf
                                <button type="submit" class="btn btn-outline-secondary btn-sm"><i class="las la-sync me-1"></i>@lang('Regenerate API key')</button>
                            </form>
                        </div>
                    </div>
                </div>

                {{-- Download your own reseller website --}}
                <div class="col-12">
                    <div class="reseller-card">
                        <div class="reseller-card__icon reseller-card__icon--success"><i class="las la-download"></i></div>
                        <div class="reseller-card__body">
                            <h3 class="reseller-card__heading">@lang('Download your own reseller website')</h3>
                            <p class="reseller-card__desc">ZIP package with your API key pre-filled. Set business name and logo in config.</p>
                            <a href="{{ route('reseller-site.download') }}?api_key={{ urlencode($reseller->api_key) }}" class="btn btn-success w-100 mb-2"><i class="las la-file-archive me-2"></i>@lang('Download ZIP')</a>
                            <a href="{{ route('user.reseller.api-docs') }}" class="btn btn-outline-primary btn-sm w-100"><i class="las la-book me-1"></i>@lang('API Documentation')</a>
                        </div>
                    </div>
                </div>

                {{-- Web installation (20k) --}}
                <div class="col-12">
                    <div class="reseller-card reseller-card--highlight">
                        ...
                        <div class="reseller-card__body flex-grow-1">
                            <div class="d-flex flex-wrap align-items-center gap-2 mb-2">
                                <h3 class="reseller-card__heading mb-0">@lang('Do you need Web installation? Pay 20k for installation on your server')</h3>
                                <span class="badge bg-dark">{{ $general->cur_sym ?? '₦' }}20,000</span>
                            </div>
                            <p class="reseller-card__desc">Submit your details and we’ll contact you for payment and setup on your server.</p>
                            <form method="post" action="{{ route('user.reseller.pro-install.submit') }}" class="reseller-form">
                                @csrf
                                <div class="row g-2">
                                    <div class="col-md-6"><label class="form-label small text-muted">@lang('Business name')</label><input type="text" name="business_name" class="form-control form-control-sm" value="{{ old('business_name', $reseller->business_name) }}" required></div>
                                    <div class="col-md-6"><label class="form-label small text-muted">@lang('Contact name')</label><input type="text" name="contact_name" class="form-control form-control-sm" value="{{ old('contact_name', $user->fullname) }}" required></div>
                                    <div class="col-md-6"><label class="form-label small text-muted">@lang('Contact email')</label><input type="email" name="contact_email" class="form-control form-control-sm" value="{{ old('contact_email', $user->email) }}" required></div>
                                    <div class="col-md-6"><label class="form-label small text-muted">@lang('Contact phone')</label><input type="text" name="contact_phone" class="form-control form-control-sm" value="{{ old('contact_phone') }}" placeholder="Optional"></div>
                                    <div class="col-12"><label class="form-label small text-muted">@lang('Message')</label><textarea name="message" class="form-control form-control-sm" rows="2" placeholder="Optional">{{ old('message') }}</textarea></div>
                                    <div class="col-12"><button type="submit" class="btn btn-dark">@lang('Request Web installation')</button></div>
                                </div>
                            </form>
                        </div>
                    </div>
